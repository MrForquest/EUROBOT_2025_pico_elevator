...
void MoveController::print_banner() {
    ...
    sleep_ms(1000);
    this->go_dist(CMD_VEL_100_MM, -1); // движение назад
}


void MoveController::build_tribune() {
    this->go_dist(CMD_VEL_100_MM, 1);
    sleep_ms(2000);
...
/*!
    ...
    @param direction направление, 1 - вперёд, 0 - назад
*/
void MoveController::go_dist(u32 time_ms, int direction) {
    rcl_ret_t ret;
    twist_msg.linear.x = 0.10 * direction;
